...
def score(
...
) -> tuple[list[tuple[JobListing, ScoredResult]], list[tuple[JobListing, FilteredResult]]]:
    ...
    return sorted(scored, key=lambda x: x[1].score, reverse=True), filtered
...
def main() -> None:
    ...
    jobs = scrape()
    ranked, filtered = score(jobs)
    report(ranked, filtered, args.output)
